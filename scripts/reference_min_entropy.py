#!/usr/bin/env python3
"""Independent min-entropy: exact largest frequency and Decimal logarithms."""
...
def entropy(counts: list[int]) -> Decimal:
    total = sum(counts)
    if not total:
        return Decimal(0)  # Project convention; no empirical maximum exists.
    largest = Fraction(max(counts), total)
    with localcontext() as context:
        context.prec = 120
        probability = Decimal(largest.numerator) / Decimal(largest.denominator)
...
def cases() -> dict[str, list[int]]:
    result = {
        ...
        "same_max_a": [6, 3, 1], "same_max_b": [6, 2, 2],
        "tied_maximum": [3, 3, 1],
...
if __name__ == "__main__":
    ...
    path = Path(__file__).resolve().parents[1] / "tests" / "data" / "min_entropy.tsv"
    expected = fixtures()
    if args.check:
        if path.read_text() != expected:
            raise SystemExit("Min-entropy reference fixtures differ; regenerate and review them.")
        print(f"{len(cases())} independent high-precision min-entropy fixtures verified.")
    else:

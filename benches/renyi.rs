...
use celandine::entropy::{renyi_entropy, renyi_entropy_distribution};
...
fn benchmarks(c: &mut Criterion) {
    ...
    for shape in ["constant", "uniform", "skewed", "mixed"] {
        for size in [
            ...
            group.bench_with_input(BenchmarkId::new(shape, size), &data, |b, data| {
                b.iter(|| black_box(renyi_entropy(black_box(data), black_box(2.0)).unwrap()));
            });
        }
    }
    group.finish();
    ...
    for size in [
        ...
        group.bench_with_input(BenchmarkId::new("mixed", size), &data, |b, data| {
            b.iter(|| black_box(renyi_entropy(black_box(data), black_box(1.0 + 1e-8)).unwrap()));
        });
    }
    group.finish();

    let mut group = c.benchmark_group("renyi_distribution");
    for shape in ["constant", "uniform", "skewed", "mixed"] {
        ...
        for (label, alpha) in [
            ...
            group.bench_function(BenchmarkId::new(shape, label), |b| {
                b.iter(|| {
                    black_box(
                        renyi_entropy_distribution(black_box(&distribution), black_box(alpha))
                            .unwrap(),
                    )
                });
            });

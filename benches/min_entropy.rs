...
use celandine::entropy::{min_entropy, min_entropy_distribution};
    ...
    for shape in ["constant", "uniform", "skewed", "mixed"] {
        for size in [16, 64, 256, 1024, 4096, 16384, 65536, 1048576, 10485760] {
            ...
            group.bench_with_input(BenchmarkId::new(shape, size), &data, |b, data| {
                b.iter(|| black_box(min_entropy(black_box(data))));
            });
        }
    }
    group.finish();

    let mut group = c.benchmark_group("min_entropy_distribution");
    for shape in ["constant", "uniform", "skewed", "mixed"] {
        let distribution = Distribution::from_bytes(&input(65536, shape));
        group.bench_function(shape, |b| {
            b.iter(|| black_box(min_entropy_distribution(black_box(&distribution))));
        });
    }
    group.finish();
}

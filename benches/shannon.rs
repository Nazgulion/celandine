...
fn benchmarks(c: &mut Criterion) {
    let mut group = c.benchmark_group("shannon_bytes");
    ...
    let mut group = c.benchmark_group("shannon_distribution");
    for shape in ["constant", "uniform", "skewed", "mixed"] {
        let distribution = Distribution::from_bytes(&input(65536, shape));
        group.bench_function(shape, |b| {
            b.iter(|| black_box(shannon_distribution(black_box(&distribution))));
        });
    }
    group.finish();
}
